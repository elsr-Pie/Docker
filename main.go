...
func main() {
	// Server for jokes
	jokeMux := http.NewServeMux()
	jokeMux.HandleFunc("/", JokeHandler)

	// Server for madlibs
	madlibMux := http.NewServeMux()
	madlibMux.HandleFunc("/", MadlibHandler)

	// Starting joke server
	go func() {
		http.ListenAndServe(":8081", jokeMux)
	}()

	// Starting madlib server
	http.ListenAndServe(":8082", madlibMux)
}
